// UI 문자열 사전. DOM을 건드리지 않으므로 빌드(Node)에서도 그대로 import할 수 있다.
// 저작권 연도를 박아두면 해가 바뀔 때마다 낡는다. 실제로 2025로 굳어 있었다.
const YEAR = new Date().getFullYear();

export const translations = {
  ko: {
    'uuid.tab.generate': '생성',
    'common.moreContent': '사용 순서와 입력 예시 더 보기',
    'dateCalc.title': '만 나이 계산기',
    'dateCalc.description': '생년월일을 넣으면 만 나이·세는 나이·연 나이를 함께 계산합니다. 날짜 차이, 디데이, 날짜 더하기도 같은 화면에서 처리합니다.',
    'dateCalc.tab.age': '만 나이',
    'dateCalc.tab.diff': '날짜 차이',
    'dateCalc.tab.dday': '디데이',
    'dateCalc.tab.add': '더하기',
    'dateCalc.age.birth': '생년월일',
    'dateCalc.age.base': '기준일 (기본: 오늘)',
    'dateCalc.age.resultTitle': '나이 결과',
    'dateCalc.age.man': '만 나이',
    'dateCalc.age.se': '세는 나이',
    'dateCalc.age.yeon': '연 나이',
    'dateCalc.age.lived': '살아온 기간',
    'dateCalc.age.livedDays': '살아온 일수',
    'dateCalc.age.nextBirthday': '다음 생일까지',
    'dateCalc.age.birthdayToday': '오늘이 생일입니다',
    'dateCalc.diff.from': '시작일',
    'dateCalc.diff.to': '종료일',
    'dateCalc.diff.resultTitle': '차이 결과',
    'dateCalc.diff.days': '차이 일수',
    'dateCalc.diff.inclusive': '당일 포함',
    'dateCalc.diff.weeks': '주 단위',
    'dateCalc.diff.ymd': '년·개월·일',
    'dateCalc.diff.weekdays': '평일 수 (주말 제외)',
    'dateCalc.diff.months': '개월 합계',
    'dateCalc.dday.target': '목표일',
    'dateCalc.dday.base': '기준일 (기본: 오늘)',
    'dateCalc.dday.resultTitle': '디데이 결과',
    'dateCalc.dday.label': '디데이',
    'dateCalc.dday.days': '남은/지난 일수',
    'dateCalc.dday.countUp': '첫날부터 몇 일째',
    'dateCalc.dday.weekday': '목표일 요일',
    'dateCalc.add.base': '기준일',
    'dateCalc.add.years': '년',
    'dateCalc.add.months': '개월',
    'dateCalc.add.weeks': '주',
    'dateCalc.add.days': '일',
    'dateCalc.add.business': '영업일',
    'dateCalc.add.hint': '음수를 넣으면 과거로 계산합니다. 영업일은 주말만 제외하며 공휴일은 반영하지 않습니다.',
    'dateCalc.add.resultTitle': '계산 결과',
    'dateCalc.add.result': '결과 날짜',
    'dateCalc.add.weekday': '요일',
    'dateCalc.add.fromToday': '오늘로부터',
    'dateCalc.add.past': '{value}일 전',
    'dateCalc.error.futureBirth': '생년월일이 기준일보다 뒤입니다.',
    'dateCalc.unit.age': '{value}세',
    'dateCalc.unit.days': '{value}일',
    'dateCalc.unit.months': '{value}개월',
    'dateCalc.unit.weeks': '{weeks}주 {days}일',
    'dateCalc.unit.ymd': '{years}년 {months}개월 {days}일',
    'dateCalc.unit.nthDay': '{value}일째',
    'dateCalc.weekday.0': '일요일',
    'dateCalc.weekday.1': '월요일',
    'dateCalc.weekday.2': '화요일',
    'dateCalc.weekday.3': '수요일',
    'dateCalc.weekday.4': '목요일',
    'dateCalc.weekday.5': '금요일',
    'dateCalc.weekday.6': '토요일',
    'home.card.datecalc.title': '만 나이 계산기',
    'home.card.datecalc.desc': '만 나이·세는 나이, 날짜 차이, 디데이 계산.',
    'page.aboutTitle': '사이트 소개',
    'page.privacyTitle': '개인정보 처리 안내',
    'page.contactTitle': '문의 안내',
    'common.languageLabel': '언어',
    'common.languageKorean': '한국어',
    'common.languageEnglish': 'English',
    'common.copy': '복사',
    'common.copySuccess': '클립보드에 복사했습니다.',
    'common.copyFail': '복사에 실패했습니다. 브라우저 권한을 확인하세요.',
    'common.clear': '지우기',
    'common.sample': '샘플',
    'common.back': '← 도구 목록으로',
    'common.shareState': '상태 공유 링크',
    'common.snippetTitle': 'CLI · 코드 스니펫',
    'common.snippetSubtitle': 'Node.js · shell',
    'common.footerHome': `© ${YEAR} stateless tools · Cloudflare Pages 배포용 정적 사이트`,
    'common.footerBase64':
      `© ${YEAR} stateless tools · 모든 변환은 브라우저에서만 처리되며 파일/쿠키를 서버로 전송하지 않습니다.`,
    'common.footerUuid': `© ${YEAR} stateless tools · Cloudflare Pages 배포용 정적 사이트`,
    'common.footerJson': `© ${YEAR} stateless tools · JSON 처리는 브라우저 메모리에서만 수행됩니다.`,
    'common.footerJwt': `© ${YEAR} stateless tools · JWT 디코딩과 JWKS 검증은 브라우저에서 처리됩니다.`,
    'common.footerSeo': `© ${YEAR} stateless tools · SEO 점검은 브라우저에서 URL 또는 HTML 소스로 실행됩니다.`,
    'common.yes': '예',
    'common.no': '아니요',
    'upload.eyebrow': 'Upload',
    'upload.titleDefault': '파일 업로드',
    'upload.choose': '파일 선택',
    'upload.replace': '다른 파일 선택',
    'upload.clear': '선택 해제',
    'upload.none': '선택된 파일이 없습니다.',
    'upload.singleSummary': '{name} · {size}',
    'upload.multiSummary': '{count}개 파일 · 총 {size}',
    'upload.extraFiles': '외 {count}개 더',
    'upload.hintAny': '로컬 파일을 끌어다 놓거나 이 기기에서 바로 선택하세요.',
    'upload.hintImage': 'PNG, JPG, WEBP, SVG 이미지를 끌어다 놓거나 이 기기에서 선택하세요.',
    'upload.hintPdfSingle': 'PDF 파일을 끌어다 놓거나 이 기기에서 선택하세요.',
    'upload.hintPdfMany': '병합할 PDF 여러 개를 끌어다 놓거나 이 기기에서 선택하세요.',
    'home.eyebrow': 'stateless dev tools',
    'home.title': '개발자 도구 모음',
    'home.description': 'UUID 변환기, Base64 디코더, JSON 포매터, JWT 디코더 같은 개발자 도구를 브라우저에서 바로 사용하세요.',
    'home.card.aihub.title': 'AI 명령어 치트시트',
    'home.card.aihub.desc': 'Claude Code와 Codex의 최신 명령 흐름과 공식 문서 링크.',
    'home.card.claude.title': 'Claude Code 명령어',
    'home.card.claude.desc': 'commands, skills, MCP, hooks, run/verify, background agents 정리.',
    'home.card.codex.title': 'Codex 명령어',
    'home.card.codex.desc': 'Codex CLI, slash commands, AGENTS.md, Skills, MCP, Automations 참고.',
    'home.card.uuid.title': 'UUID 변환기',
    'home.card.uuid.desc': 'UUID 생성, UUID hex(binary16) 변환, ULID 생성까지 한 화면에서 처리.',
    'home.card.base64.title': 'Base64 인코더/디코더',
    'home.card.base64.desc': 'Base64 인코딩, 디코딩, URL-safe Base64 변환.',
    'home.card.json.title': 'JSON 포매터/검증기',
    'home.card.json.desc': 'JSON 포맷팅, 검증, 압축, 트리 보기까지 한번에.',
    'home.card.jwt.title': 'JWT 디코더/검사기',
    'home.card.jwt.desc': 'JWT 디코드, claims 확인, exp/nbf 만료 점검.',
    'home.card.url.title': 'URL 인코더/디코더',
    'home.card.url.desc': 'URL 인코딩, URL 디코딩, 쿼리 파라미터 확인.',
    'home.card.hash.title': 'SHA 해시 계산기',
    'home.card.hash.desc': 'SHA-1/256/384/512 해시 계산과 HEX/BASE64 출력.',
    'home.card.cron.title': 'Cron 표현식 파서',
    'home.card.cron.desc': 'Cron 해석과 다음 실행 시간 계산.',
    'home.card.timestamp.title': 'Unix 타임스탬프 변환기',
    'home.card.timestamp.desc': 'epoch 초/밀리초와 날짜를 서로 변환.',
    'home.card.password.title': '비밀번호 생성기',
    'home.card.password.desc': '길이/문자셋 옵션으로 보안 비밀번호를 생성하고 강도를 확인하세요.',
    'home.card.regex.title': '정규식 테스트기',
    'home.card.regex.desc': '정규식 패턴, 플래그, 치환 결과 테스트.',
    'home.card.qr.title': 'QR 코드 생성기/스캐너',
    'home.card.qr.desc': 'QR 코드 생성과 이미지 QR 스캔.',
    'home.card.diff.title': '텍스트 비교기',
    'home.card.diff.desc': '두 텍스트를 줄 단위로 비교.',
    'home.card.color.title': '색상 변환기/대비 검사기',
    'home.card.color.desc': 'HEX/RGB/HSL 변환과 대비 비율 계산.',
    'home.card.markdown.title': 'Markdown 미리보기',
    'home.card.markdown.desc': 'Markdown 렌더링과 HTML 변환 결과 확인.',
    'home.card.convert.title': '단위 변환기',
    'home.card.convert.desc': '길이/무게/온도 단위 빠른 변환.',
    'home.card.filehash.title': '파일 해시 계산기',
    'home.card.filehash.desc': '업로드 파일의 SHA 해시 계산.',
    'home.card.imagebase64.title': '이미지 Base64 변환기',
    'home.card.imagebase64.desc': '이미지 파일과 Base64 data URL 상호 변환.',
    'home.card.case.title': '텍스트 케이스 변환기',
    'home.card.case.desc': 'camelCase, snake_case, kebab-case, PascalCase 변환.',
    'home.card.jsonyaml.title': 'JSON YAML 변환기',
    'home.card.jsonyaml.desc': 'JSON과 YAML 상호 변환, CSV 정리.',
    'home.card.query.title': 'URL 쿼리 생성기',
    'home.card.query.desc': 'URL 쿼리 파라미터 생성과 인코딩.',
    'home.card.ipua.title': 'IP/User-Agent 확인',
    'home.card.ipua.desc': 'IP 주소, UA, 언어, 타임존 빠른 확인.',
    'home.card.textstats.title': '글자수 세기',
    'home.card.textstats.desc': '공백 포함/제외 글자수, 단어·문장·문단, 원고지 분량.',
    'home.card.ipcidr.title': 'CIDR 계산기',
    'home.card.ipcidr.desc': 'CIDR 기반 네트워크, 브로드캐스트, 호스트 범위 계산.',
    'home.card.pdf.title': 'PDF 병합/분할 툴',
    'home.card.pdf.desc': 'PDF 병합, 분할, 페이지 추출, 워터마크.',
    'home.card.imageoptimize.title': '이미지 최적화',
    'home.card.imageoptimize.desc': '압축, 리사이즈, WEBP/JPEG/PNG 변환.',
    'home.card.ocr.title': 'OCR 텍스트 추출',
    'home.card.ocr.desc': '이미지에서 한국어/영어 텍스트 추출.',
    'home.card.seocheck.title': 'SEO 메타 태그 검사기',
    'home.card.seocheck.desc': 'SEO 메타 태그 검사와 Title/Description/OG 점검.',
    'home.card.utm.title': 'UTM 생성기',
    'home.card.utm.desc': 'UTM 파라미터 캠페인 URL 생성.',
    'home.card.textcleaner.title': '텍스트 정리기',
    'home.card.textcleaner.desc': '중복 제거, 공백 정리, 정렬, slugify.',
    'home.card.apitester.title': 'API 요청 테스트',
    'home.card.apitester.desc': 'HTTP 요청 테스트, 응답 확인, cURL 변환.',
    'timestamp.title': 'Unix 타임스탬프 변환기',
    'timestamp.description': '유닉스 타임스탬프 변환기에서 epoch 초/밀리초와 날짜를 서로 바꿔 확인하세요.',
    'timestamp.current.title': '현재 시간',
    'timestamp.current.unix': 'Unix 타임스탬프 (초)',
    'timestamp.current.iso': 'ISO 8601',
    'timestamp.current.stop': '업데이트 중지',
    'timestamp.current.start': '업데이트 시작',
    'timestamp.convert.title': '변환기',
    'timestamp.input.label': 'Unix 타임스탬프',
    'timestamp.input.current': '현재 시간 입력',
    'timestamp.unit.seconds': '초 (Seconds)',
    'timestamp.unit.millis': '밀리초 (ms)',
    'timestamp.output.local': '로컬 시간',
    'timestamp.output.iso': 'ISO / UTC',
    'timestamp.output.relative': '상대 시간',
    'timestamp.reverse.title': '날짜 → 타임스탬프',
    'timestamp.reverse.button': '변환',
    'base64.title': 'Base64 인코더/디코더',
    'base64.description': 'Base64 인코딩과 디코딩, URL-safe Base64 변환, 파일 Base64 변환을 브라우저에서 처리합니다.',
    'base64.encode.title': '문자열 → Base64',
    'base64.encode.input': '문자열 입력',
    'base64.encode.placeholder': '예: 안녕하세요',
    'base64.encode.urlSafe': 'URL-safe 변환',
    'base64.encode.button': '인코딩',
    'base64.encode.result': 'Base64 결과',
    'base64.decode.title': 'Base64 → 문자열',
    'base64.decode.input': 'Base64 입력',
    'base64.decode.placeholder': '예: 7JWI64WV7ZWY7IS47JqU',
    'base64.decode.strip': '공백 제거',
    'base64.decode.urlSafe': 'URL-safe 복구',
    'base64.decode.button': '디코딩',
    'base64.decode.result': '문자열 결과',
    'base64.file.title': '파일 ↔ Base64',
    'base64.file.helper': '파일을 선택하면 즉시 Base64로 변환됩니다.',
    'base64.file.downloadPlaceholder': 'decoded.bin',
    'base64.file.downloadBtn': 'Base64 → 파일 저장',
    'uuid.title': 'UUID 변환기',
    'uuid.description': 'UUID 생성, UUID hex(binary16) 변환, ULID 생성까지 한 화면에서 처리하는 UUID 변환기입니다.',
    'uuid.random.label': '랜덤 UUID',
    'uuid.random.button': '새로 생성',
    'uuid.random.mode': '생성 옵션',
    'uuid.random.v4': 'UUID v4',
    'uuid.random.v7': 'UUID v7 (시간순)',
    'uuid.random.ulid': 'ULID (가독성)',
    'uuid.toHex.title': 'UUID → Hex(binary16)',
    'uuid.toHex.input': 'UUID 입력',
    'uuid.toHex.placeholder': 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx',
    'uuid.toHex.button': '변환',
    'uuid.toHex.output': 'Hex 결과',
    'uuid.toUuid.title': 'Hex(binary16) → UUID',
    'uuid.toUuid.input': 'Hex 입력 (32자리)',
    'uuid.toUuid.placeholder': '32자리 16진수',
    'uuid.toUuid.button': '복구',
    'uuid.toUuid.output': 'UUID 결과',
    'uuid.bulk.title': '대량 생성',
    'uuid.bulk.count': '개수',
    'uuid.bulk.button': '생성',
    'json.title': 'JSON 포매터/검증기',
    'json.description': 'JSON 포맷팅, JSON 검증, JSON 압축, 트리 보기까지 빠르게 확인하는 JSON 포매터입니다.',
    'json.input': '입력 JSON',
    'json.placeholder': '{"hello":"world"}',
    'json.format': '포맷',
    'json.minify': '압축',
    'json.output': '출력',
    'json.searchLabel': '키/값 검색',
    'json.searchPlaceholder': 'user.id',
    'json.highlightTitle': '하이라이트',
    'json.treeTitle': '트리 뷰',
    'json.treeCollapse': '모두 접기',
    'json.treeNode.array': '배열',
    'json.treeNode.object': '객체',
    'json.meta.empty': '입력 없음',
    'messages.base64.encodeSuccess': '문자열을 Base64로 인코딩했습니다.',
    'messages.base64.encodeError': '인코딩 중 오류가 발생했습니다.',
    'messages.base64.decodeSuccess': 'Base64를 문자열로 디코딩했습니다.',
    'messages.base64.decodeError': '디코딩 중 오류가 발생했습니다. 입력을 확인하세요.',
    'messages.base64.fileEncoded': '{name} 파일을 Base64로 변환했습니다.',
    'messages.base64.fileDecoded': 'Base64를 파일로 내보냈습니다.',
    'messages.base64.fileError': '파일 변환에 실패했습니다.',
    'messages.uuid.randomGenerated': '새 UUID를 생성했습니다.',
    'messages.uuid.toHexSuccess': 'UUID를 hex(binary16)으로 변환했습니다.',
    'messages.uuid.toHexError': 'UUID 형식이 올바르지 않습니다.',
    'messages.uuid.toUuidSuccess': 'hex(binary16)을 UUID로 복구했습니다.',
    'messages.uuid.toUuidError': 'Hex(binary16)은 32자리 16진수여야 합니다.',
    'messages.uuid.bulkGenerated': '{count}개의 식별자를 생성했습니다.',
    'messages.json.empty': 'JSON 입력이 비었습니다.',
    'messages.json.pretty': 'JSON을 보기 좋게 정렬했습니다.',
    'messages.json.compact': 'JSON을 압축했습니다.',
    'messages.json.error': 'JSON 파싱 실패: {message}',
    'messages.json.cleared': '입력을 초기화했습니다.',
    'messages.json.sample': '샘플 JSON을 불러왔습니다.',
    'json.meta.array': '배열 · {count} items',
    'json.meta.object': '객체 · {count} keys ({examples})',
    'json.meta.primitive': '프리미티브 · {type}',
    'jwt.title': 'JWT 디코더/검사기',
    'jwt.description': 'JWT 디코딩, claims 확인, exp/nbf 만료 점검, JWKS 기반 서명 검증을 지원합니다.',
    'jwt.input': 'JWT 토큰',
    'jwt.placeholder': 'eyJhbGciOi...',
    'jwt.action.decode': '디코드',
    'jwt.output.header': '헤더',
    'jwt.output.payload': '페이로드',
    'jwt.success': 'JWT를 디코드했습니다.',
    'jwt.error.empty': 'JWT 값을 입력하세요.',
    'jwt.error.format': 'JWT 형식이 올바르지 않습니다.',
    'jwt.error.decode': 'JWT 디코딩에 실패했습니다.',
    'jwt.meta.expires': '만료: {date}',
    'jwt.meta.expired': '만료됨: {date}',
    'jwt.meta.nbfActive': 'nbf 통과: {date}',
    'jwt.meta.nbfPending': 'nbf 대기: {date}',
    'cron.title': 'Cron 표현식 파서',
    'cron.description': 'Cron 표현식을 해석하고 다음 실행 시간을 계산하는 Cron 파서입니다.',
    'cron.input': 'Cron 표현식',
    'cron.nextRuns': '다음 실행 시간 (5회)',
    'cron.empty': 'Cron 표현식을 입력하세요. 예: */5 * * * *',
    'cron.error.invalid': '유효하지 않은 Cron 표현식입니다.',
    'cron.error.details': '오류 상세: {message}',
    'url.title': 'URL 인코더/디코더',
    'url.description': 'URL 인코딩/디코딩과 쿼리 스트링 파싱을 한 번에 처리하는 URL 변환기입니다.',
    'url.tab.convert': '인코딩 / 디코딩',
    'url.tab.build': '쿼리 생성',
    'url.input': '원본 문자열/URL',
    'url.placeholder': 'https://example.com?q=안녕하세요',
    'url.encode': '인코딩',
    'url.decode': '디코딩',
    'url.output': '결과',
    'url.outputHelper': '복사하거나 파라미터 테이블을 확인하세요.',
    'url.paramsTitle': '쿼리 파라미터',
    'url.paramsEmpty': '표시할 파라미터가 없습니다.',
    'url.success.encode': 'URL을 인코딩했습니다.',
    'url.success.decode': 'URL을 디코딩했습니다.',
    'url.error.empty': '값을 입력하세요.',
    'url.error.encode': '인코딩에 실패했습니다.',
    'url.error.decode': '디코딩에 실패했습니다.',
    'hash.title': 'SHA 해시 계산기',
    'hash.description': 'SHA-1, SHA-256, SHA-384, SHA-512 값을 계산하는 SHA 해시 계산기입니다.',
    'hash.input': '입력 문자열',
    'hash.placeholder': 'stateless tools',
    'hash.algorithm': '알고리즘',
    'hash.uppercase': 'HEX 대문자 출력',
    'hash.compute': '해시 계산',
    'hash.hex': 'HEX',
    'hash.base64': 'Base64',
    'hash.error.empty': '입력 문자열이 필요합니다.',
    'hash.error.unsupported': '이 브라우저에서 해시 계산을 완료할 수 없습니다.',
    'hash.success': '{algorithm} 해시를 계산했습니다.',
    'password.title': '비밀번호 생성기',
    'password.description': '랜덤 비밀번호 생성과 길이·문자셋 옵션, 강도 확인을 지원합니다.',
    'password.length': '길이',
    'password.count': '생성 개수',
    'password.option.lower': '소문자 (a-z)',
    'password.option.upper': '대문자 (A-Z)',
    'password.option.numbers': '숫자 (0-9)',
    'password.option.symbols': '특수문자',
    'password.option.excludeAmbiguous': '헷갈리는 문자 제외 (0/O, l/1)',
    'password.generate': '생성',
    'password.output': '생성 결과',
    'password.strength.label': '강도: {level} ({bits} bits)',
    'password.strength.weak': '약함',
    'password.strength.fair': '보통',
    'password.strength.good': '좋음',
    'password.strength.strong': '강력',
    'password.strength.excellent': '매우 강력',
    'password.error.charset': '최소 1개 이상의 문자셋을 선택하세요.',
    'password.success.generated': '{count}개 비밀번호를 생성했습니다.',
    'regex.title': '정규식 테스트기',
    'regex.description': '정규식 패턴, 플래그, 치환 결과를 테스트하는 정규식 테스트기입니다.',
    'regex.pattern': '패턴',
    'regex.pattern.placeholder': '\\b\\w+@\\w+\\.\\w+\\b',
    'regex.flags': '플래그',
    'regex.input': '테스트 텍스트',
    'regex.test': '테스트',
    'regex.output': '하이라이트 결과',
    'regex.matches': '매치 목록',
    'regex.error.pattern': '정규식 패턴을 입력하세요.',
    'regex.error.invalid': '정규식 오류: {message}',
    'regex.result.none': '매치 결과가 없습니다.',
    'regex.result.count': '{count}개 매치를 찾았습니다.',
    'case.title': '텍스트 케이스 변환기',
    'case.description': 'camelCase, snake_case, kebab-case, PascalCase를 바꾸는 텍스트 케이스 변환기입니다.',
    'case.input': '입력 텍스트',
    'case.run': '변환',
    'case.error.empty': '텍스트를 입력하세요.',
    'case.success': '변환 완료.',
    'jsonyaml.title': 'JSON YAML 변환기',
    'jsonyaml.description': 'JSON과 YAML을 서로 바꾸고 CSV 변환까지 처리하는 JSON YAML 변환기입니다.',
    'jsonyaml.input': '입력',
    'jsonyaml.output': '출력',
    'jsonyaml.toYaml': 'JSON → YAML',
    'jsonyaml.toJson': 'YAML → JSON',
    'jsonyaml.success.toYaml': 'JSON → YAML 변환 완료.',
    'jsonyaml.success.toJson': 'YAML → JSON 변환 완료.',
    'jsonyaml.error': '변환 실패: {message}',
    'query.base': 'Base URL',
    'query.add': '파라미터 추가',
    'query.build': 'URL 생성',
    'query.output': '결과 URL',
    'query.remove': '삭제',
    'query.success': 'URL 생성 완료.',
    'query.error.base': '기본 URL 형식이 잘못되었습니다.',
    'ipua.title': 'IP/User-Agent 확인',
    'ipua.description': 'IP 주소, User-Agent, 언어, 타임존을 빠르게 확인하는 브라우저 환경 점검 도구입니다.',
    'ipua.externalNote': '공인 IP는 브라우저가 스스로 알 수 없어 api.ipify.org를 호출합니다. 이때 접속 IP가 해당 서비스로 전달됩니다. User-Agent·언어·시간대·플랫폼은 브라우저에서 바로 읽습니다.',
    'ipua.refresh': '새로고침',
    'ipua.lang': 'Language',
    'ipua.tz': 'Timezone',
    'ipua.platform': 'Platform',
    'ipua.success': '정보 갱신 완료.',
    'ipua.error.ip': 'IP 조회 실패(네트워크/CORS).',
    'textStats.title': '글자수 세기',
    'textStats.description': '공백 포함/제외 글자수와 단어·문장·문단 수, 원고지 분량을 입력하는 즉시 계산합니다.',
    'textStats.input': '입력 텍스트',
    'textStats.run': '통계 계산',
    'textStats.sample': '샘플',
    'textStats.resultTitle': '통계 결과',
    'textStats.charsWithSpaces': '문자 수 (공백 포함)',
    'textStats.charsNoSpaces': '문자 수 (공백 제외)',
    'textStats.words': '단어 수',
    'textStats.lines': '행 수',
    'textStats.paragraphs': '문단 수',
    'textStats.sentences': '문장 수(간이)',
    'textStats.readingTime': '예상 읽기 시간',
    'textStats.bytes': '바이트',
    'textStats.manuscript': '원고지(200자)',
    'textStats.uniqueWords': '고유 단어',
    'textStats.success': '통계 계산 완료.',
    'ipCidr.title': 'CIDR 계산기',
    'ipCidr.description': 'IPv4 CIDR 블록에서 네트워크, 브로드캐스트, 호스트 범위를 계산합니다.',
    'ipCidr.ip': 'IPv4 주소',
    'ipCidr.prefix': 'CIDR 프리픽스',
    'ipCidr.calculate': '계산',
    'ipCidr.copy': 'CIDR 복사',
    'ipCidr.output': '결과',
    'ipCidr.network': '네트워크 주소',
    'ipCidr.broadcast': '브로드캐스트',
    'ipCidr.netmask': '넷마스크',
    'ipCidr.wildcard': '와일드카드',
    'ipCidr.usable': '사용 가능한 호스트',
    'ipCidr.class': '주소 분류',
    'ipCidr.firstHost': '최초 호스트',
    'ipCidr.lastHost': '최종 호스트',
    'ipCidr.total': '총 주소 수',
    'ipCidr.isNetwork': '입력 IP가 네트워크 주소',
    'ipCidr.isBroadcast': '입력 IP가 브로드캐스트 주소',
    'ipCidr.error.ip': '유효한 IPv4 주소를 입력하세요.',
    'ipCidr.error.prefix': '프리픽스는 0~32 사이여야 합니다.',
    'ipCidr.success': 'CIDR 계산이 완료되었습니다.',
    'qr.title': 'QR 코드 생성기/스캐너',
    'qr.description': 'QR 코드 생성과 이미지 QR 스캔을 한 화면에서 처리하는 QR 코드 도구입니다.',
    'qr.type': '타입',
    'qr.type.text': '텍스트/URL',
    'qr.type.wifi': 'Wi-Fi',
    'qr.input': '텍스트/URL',
    'qr.ssid': 'SSID',
    'qr.password': '비밀번호',
    'qr.security': '보안',
    'qr.security.none': '없음',
    'qr.size': '크기(px)',
    'qr.generate': '생성',
    'qr.download': 'PNG 다운로드',
    'qr.scanFile': '이미지에서 스캔',
    'qr.scanOutput': '스캔 결과',
    'qr.error.empty': '텍스트를 입력하세요.',
    'qr.error.lib': 'QR 라이브러리를 불러오지 못했습니다.',
    'qr.error.generate': 'QR 생성에 실패했습니다.',
    'qr.success.generated': 'QR 코드를 생성했습니다.',
    'qr.error.unsupported': '이 브라우저는 QR 스캔을 지원하지 않습니다.',
    'qr.error.notFound': 'QR 코드를 찾지 못했습니다.',
    'qr.success.scanned': 'QR 스캔 완료.',
    'qr.error.scan': 'QR 스캔 처리 중 오류가 발생했습니다.',
    'diff.title': '텍스트 비교기',
    'diff.description': '두 텍스트를 줄 단위로 비교하는 텍스트 비교기입니다.',
    'diff.left': '원본',
    'diff.right': '수정본',
    'diff.compare': '비교',
    'diff.result': '결과',
    'diff.unified': 'Unified Diff',
    'diff.result.changed': '변경 라인 {count}개',
    'diff.result.same': '차이가 없습니다.',
    'color.title': '색상 변환기/대비 검사기',
    'color.description': 'HEX, RGB, HSL 색상 변환과 대비 비율을 확인하는 색상 변환기입니다.',
    'color.pick': '색상 선택',
    'color.contrast': '대비 비율',
    'color.contrast.value': 'vs White: {white} / vs Black: {black}',
    'color.success': '색상 값 변환 완료.',
    'color.error.parse': '색상 파싱 오류',
    'markdown.title': 'Markdown 미리보기',
    'markdown.description': 'Markdown 렌더링과 HTML 출력을 확인하는 Markdown 미리보기 도구입니다.',
    'markdown.input': 'Markdown',
    'markdown.render': '렌더',
    'markdown.preview': '미리보기',
    'markdown.error.lib': 'Markdown 라이브러리를 불러오지 못했습니다.',
    'markdown.success.render': '렌더링 완료.',
    'markdown.error.render': '렌더링 실패',
    'convert.title': '단위 변환기',
    'convert.description': '길이, 무게, 온도 단위를 빠르게 바꾸는 단위 변환기입니다.',
    'convert.category': '카테고리',
    'convert.category.length': '길이',
    'convert.category.weight': '무게',
    'convert.category.temp': '온도',
    'convert.input': '입력 값',
    'convert.from': 'From',
    'convert.to': 'To',
    'convert.swap': '교환',
    'convert.run': '변환',
    'convert.output': '결과',
    'convert.error.nan': '숫자를 입력하세요.',
    'convert.success': '변환 완료.',
    'filehash.title': '파일 해시 계산기',
    'filehash.description': '업로드한 파일의 SHA 해시를 계산하는 파일 해시 계산기입니다.',
    'filehash.file': '파일',
    'filehash.algorithm': '알고리즘',
    'filehash.run': '계산',
    'filehash.error.file': '파일을 선택하세요.',
    'filehash.success': '{algorithm} 계산 완료 ({name})',
    'filehash.error.run': '해시 계산 실패',
    'imagebase64.title': '이미지 Base64 변환기',
    'imagebase64.description': '이미지를 data URL로 바꾸거나 Base64 이미지 문자열을 복원합니다.',
    'imagebase64.file': '이미지 파일',
    'imagebase64.encode': 'Base64 변환',
    'imagebase64.decode': '파일로 저장',
    'imagebase64.output': 'data URL',
    'imagebase64.error.file': '이미지를 선택하세요.',
    'imagebase64.success.encode': 'Base64 변환 완료.',
    'imagebase64.error.encode': '변환 실패',
    'imagebase64.error.format': '유효한 data URL 형식이 아닙니다.',
    'imagebase64.success.decode': '파일 저장 완료.',
    'pdf.title': 'PDF 병합/분할 툴',
    'pdf.description': 'PDF 병합, PDF 분할, 페이지 추출, 워터마크를 브라우저에서 처리합니다.',
    'pdf.mergeSection': '병합',
    'pdf.mergeDescription': '여러 PDF를 업로드 순서대로 병합합니다.',
    'pdf.mergeFiles': '병합할 PDF 파일',
    'pdf.singleSection': '단일 PDF 처리',
    'pdf.singleDescription': '하나의 파일로 분할, 워터마크, 메타데이터, 텍스트 추출을 처리합니다.',
    'pdf.file': 'PDF 파일',
    'pdf.mergeBtn': '병합',
    'pdf.splitBtn': '범위 추출',
    'pdf.watermarkBtn': '워터마크',
    'pdf.metadataBtn': '메타데이터 조회',
    'pdf.extractTextBtn': '텍스트 추출',
    'pdf.fromPage': '시작 페이지',
    'pdf.toPage': '종료 페이지',
    'pdf.watermarkText': '워터마크 텍스트',
    'pdf.dropHint': '여기로 파일을 끌어다 놓으세요.',
    'pdf.mergeDropHint': '또는 클릭해서 파일을 선택하세요.',
    'pdf.singleDropHint': '또는 클릭해서 파일을 선택하세요.',
    'pdf.fileNone': '선택된 파일이 없습니다.',
    'pdf.fileSummaryOne': '{name}',
    'pdf.fileSummaryMany': '{count}개 파일이 선택됨',
    'pdf.moreFiles': '외 {count}개 더',
    'pdf.fileSummaryHeader': '{count}개 파일',
    'pdf.fileSizeLabel': '총 {size}',
    'pdf.fileMeta': '{name} · {size}',
    'pdf.output': '결과',
    'pdf.downloadBtn': '다운로드',
    'pdf.page': '페이지',
    'pdf.emptyText': '(텍스트 없음)',
    'pdf.meta.filename': '파일명',
    'pdf.meta.size': '크기',
    'pdf.meta.pages': '페이지 수',
    'pdf.meta.encrypted': '암호화',
    'pdf.meta.title': '제목',
    'pdf.meta.author': '저자',
    'pdf.meta.subject': '주제',
    'pdf.meta.keywords': '키워드',
    'pdf.meta.creator': '생성자',
    'pdf.meta.producer': '제작자',
    'pdf.meta.created': '생성일',
    'pdf.meta.modified': '수정일',
    'pdf.meta.valueUnknown': '알 수 없음',
    'pdf.meta.valueNone': '미입력',
    'messages.pdf.needTwoFiles': '병합하려면 PDF 파일을 2개 이상 선택하세요.',
    'messages.pdf.requireOneFile': 'PDF 파일 1개를 먼저 선택하세요.',
    'messages.pdf.invalidRange': '시작 페이지는 종료 페이지보다 클 수 없습니다.',
    'messages.pdf.emptyWatermark': '워터마크 텍스트를 입력하세요.',
    'messages.pdf.mergeDone': 'PDF 병합 완료.',
    'messages.pdf.splitDone': '페이지 범위 추출 완료.',
    'messages.pdf.watermarkDone': '워터마크 적용 완료.',
    'messages.pdf.metadataDone': '메타데이터 조회 완료.',
    'messages.pdf.extractDone': '{from}~{to} 페이지 텍스트 추출 완료.',
    'messages.pdf.singleFileOnly': '단일 PDF 기능은 파일을 1개만 선택해야 합니다.',
    'messages.pdf.invalidFileType': 'PDF 파일이 아닌 파일은 제외했습니다. PDF만 업로드할 수 있습니다.',
    'messages.pdf.mergeSuccess': '{count}개 파일 병합 완료, 총 {pages}페이지',
    'messages.pdf.splitSummary': '{from}~{to} / 총 {total} 페이지',
    'messages.pdf.watermarkSummary': '{pages}페이지 적용',
    'messages.pdf.mergeError': 'PDF 병합 중 오류가 발생했습니다.',
    'messages.pdf.splitError': 'PDF 페이지 추출 중 오류가 발생했습니다.',
    'messages.pdf.watermarkError': '워터마크 적용 중 오류가 발생했습니다.',
    'messages.pdf.metadataError': 'PDF 메타데이터 조회 중 오류가 발생했습니다.',
    'messages.pdf.extractError': 'PDF 텍스트 추출 중 오류가 발생했습니다.',
    'api.title': 'API 요청 테스트',
    'api.description': 'HTTP 요청을 보내고 응답을 확인한 뒤 cURL로 내보내는 API 요청 테스트 도구입니다.',
    'api.headers': '헤더 (Key: Value)',
    'api.body': '본문',
    'api.send': '요청 전송',
    'api.response': '응답',
    'messages.api.urlRequired': '요청 URL을 입력하세요.',
    'messages.api.done': 'API 요청 완료.',
    'messages.api.failed': '요청 실패: {message}',
    'imageOptimize.title': '이미지 최적화',
    'imageOptimize.description': '압축, 리사이즈, JPEG/PNG/WEBP 변환을 브라우저에서 처리합니다.',
    'imageOptimize.file': '이미지 파일',
    'imageOptimize.maxWidth': '최대 너비(px)',
    'imageOptimize.quality': '품질(0.2~1)',
    'imageOptimize.format': '출력 포맷',
    'imageOptimize.run': '최적화',
    'imageOptimize.download': '다운로드',
    'imageOptimize.before': '원본',
    'imageOptimize.after': '최적화 결과',
    'imageOptimize.preview': '미리보기',
    'messages.imageOptimize.selectFile': '이미지를 선택하세요.',
    'messages.imageOptimize.done': '이미지 최적화 완료.',
    'messages.imageOptimize.failed': '이미지 최적화에 실패했습니다.',
    'messages.imageOptimize.reduced': '절감',
    'ocr.title': 'OCR 텍스트 추출',
    'ocr.description': '이미지에서 한글과 영어 텍스트를 추출하는 OCR 도구입니다.',
    'ocr.file': '이미지 파일',
    'ocr.language': '언어',
    'ocr.langBoth': '한국어 + 영어',
    'ocr.langEnglish': '영어',
    'ocr.langKorean': '한국어',
    'ocr.run': 'OCR 실행',
    'ocr.output': '추출 결과',
    'messages.ocr.selectFile': 'OCR 처리할 이미지를 선택하세요.',
    'messages.ocr.loading': 'OCR 엔진을 불러오는 중입니다.',
    'messages.ocr.done': 'OCR 완료.',
    'messages.ocr.failed': 'OCR 처리에 실패했습니다.',
    'seo.title': 'SEO 메타 태그 검사기',
    'seo.description': 'Title, Description, OG, Canonical을 점검하는 SEO 메타 태그 검사기입니다.',
    'seo.tab.serp': '제목·설명 검사',
    'seo.tab.html': 'HTML 전체 점검',
    'seo.titleLabel': '제목 (title 태그)',
    'seo.descLabel': '설명 (meta description)',
    'seo.urlLabel': '표시 URL (선택)',
    'seo.previewLabel': '검색결과 미리보기',
    'seo.liveNote': '입력하는 대로 즉시 계산합니다. 버튼을 누를 필요가 없습니다.',
    'seo.html': 'HTML 소스(선택)',
    'seo.run': '점검',
    'seo.output': '결과',
    'seo.report.title': 'Title',
    'seo.report.description': 'Description',
    'seo.report.h1': 'H1',
    'seo.report.canonical': 'Canonical',
    'seo.report.ogTitle': 'OG Title',
    'seo.report.ogDescription': 'OG Description',
    'seo.report.robots': 'Robots',
    'seo.report.jsonLdBlocks': 'JSON-LD blocks',
    'messages.seo.fetchFailed': 'URL 직접 조회에 실패했습니다(CORS 가능성). HTML 소스를 붙여 넣어 검사해 보세요.',
    'messages.seo.needInput': 'URL 또는 HTML 소스를 입력하세요.',
    'messages.seo.done': 'SEO 점검 완료.',
    'messages.seo.failed': 'SEO 점검에 실패했습니다.',
    'utm.title': 'UTM 생성기',
    'utm.description': 'UTM 파라미터를 붙인 캠페인 URL을 만드는 UTM 생성기입니다.',
    'utm.build': 'URL 생성',
    'utm.shortOutput': '짧은 URL(시뮬레이션)',
    'messages.utm.done': 'UTM URL 생성 완료.',
    'messages.utm.invalidBase': '기본 URL이 올바르지 않습니다.',
    'messages.utm.failed': 'UTM URL 생성에 실패했습니다.',
    'textCleaner.title': '텍스트 정리기',
    'textCleaner.description': '공백 정리, 중복 제거, 정렬, slugify를 한 번에 처리합니다.',
    'textCleaner.input': '입력 텍스트',
    'textCleaner.trim': '앞뒤 공백 제거',
    'textCleaner.removeEmpty': '빈 줄 제거',
    'textCleaner.dedupe': '중복 줄 제거',
    'textCleaner.sort': '정렬',
    'textCleaner.slug': 'slug 변환',
    'textCleaner.run': '정리 실행',
    'textCleaner.output': '출력',
    'messages.textCleaner.done': '텍스트 정리 완료.',
    'textStats.placeholder': '여기에 텍스트를 붙여넣으세요.',
    'textStats.sampleText': '안녕하세요. Quick brown fox jumps over the lazy dog.\n\nThis is a sample text for counting characters and words.\n여러 줄과 문장을 함께 계산합니다.',
    'textStats.readingTimeValue': '{minutes}분 {seconds}초',
    'jsonyaml.success.csvToJson': 'CSV를 JSON으로 변환했습니다.',
    'jsonyaml.success.jsonToCsv': 'JSON을 CSV로 변환했습니다.',
    'jwt.verify.enterJwks': 'JWKS URL을 입력하세요.',
    'jwt.verify.noKey': 'JWKS에서 일치하는 키를 찾지 못했습니다.',
    'jwt.verify.success': 'JWT 서명 검증 성공.',
    'jwt.verify.failed': 'JWT 서명 검증 실패: {message}',
  },
  en: {
    'uuid.tab.generate': 'Generate',
    'common.moreContent': 'More: step-by-step usage and worked examples',
    'dateCalc.title': 'Age Calculator',
    'dateCalc.description': 'Get an exact age from a birth date, plus date differences, D-day countdowns and date arithmetic on one screen.',
    'dateCalc.tab.age': 'Age',
    'dateCalc.tab.diff': 'Date difference',
    'dateCalc.tab.dday': 'D-day',
    'dateCalc.tab.add': 'Add',
    'dateCalc.age.birth': 'Birth date',
    'dateCalc.age.base': 'As of (defaults to today)',
    'dateCalc.age.resultTitle': 'Age result',
    'dateCalc.age.man': 'Exact age',
    'dateCalc.age.se': 'Korean age (counting)',
    'dateCalc.age.yeon': 'Year age',
    'dateCalc.age.lived': 'Time lived',
    'dateCalc.age.livedDays': 'Days lived',
    'dateCalc.age.nextBirthday': 'Next birthday in',
    'dateCalc.age.birthdayToday': 'Today is the birthday',
    'dateCalc.diff.from': 'Start date',
    'dateCalc.diff.to': 'End date',
    'dateCalc.diff.resultTitle': 'Difference',
    'dateCalc.diff.days': 'Days between',
    'dateCalc.diff.inclusive': 'Including both days',
    'dateCalc.diff.weeks': 'In weeks',
    'dateCalc.diff.ymd': 'Years / months / days',
    'dateCalc.diff.weekdays': 'Weekdays (no weekends)',
    'dateCalc.diff.months': 'Total months',
    'dateCalc.dday.target': 'Target date',
    'dateCalc.dday.base': 'As of (defaults to today)',
    'dateCalc.dday.resultTitle': 'D-day result',
    'dateCalc.dday.label': 'D-day',
    'dateCalc.dday.days': 'Days remaining / past',
    'dateCalc.dday.countUp': 'Day number since start',
    'dateCalc.dday.weekday': 'Target weekday',
    'dateCalc.add.base': 'Base date',
    'dateCalc.add.years': 'Years',
    'dateCalc.add.months': 'Months',
    'dateCalc.add.weeks': 'Weeks',
    'dateCalc.add.days': 'Days',
    'dateCalc.add.business': 'Business days',
    'dateCalc.add.hint': 'Negative values move backwards. Business days skip weekends only, not public holidays.',
    'dateCalc.add.resultTitle': 'Result',
    'dateCalc.add.result': 'Result date',
    'dateCalc.add.weekday': 'Weekday',
    'dateCalc.add.fromToday': 'From today',
    'dateCalc.add.past': '{value} days ago',
    'dateCalc.error.futureBirth': 'The birth date is after the reference date.',
    'dateCalc.unit.age': '{value}',
    'dateCalc.unit.days': '{value} days',
    'dateCalc.unit.months': '{value} months',
    'dateCalc.unit.weeks': '{weeks}w {days}d',
    'dateCalc.unit.ymd': '{years}y {months}m {days}d',
    'dateCalc.unit.nthDay': 'day {value}',
    'dateCalc.weekday.0': 'Sunday',
    'dateCalc.weekday.1': 'Monday',
    'dateCalc.weekday.2': 'Tuesday',
    'dateCalc.weekday.3': 'Wednesday',
    'dateCalc.weekday.4': 'Thursday',
    'dateCalc.weekday.5': 'Friday',
    'dateCalc.weekday.6': 'Saturday',
    'home.card.datecalc.title': 'Age Calculator',
    'home.card.datecalc.desc': 'Exact age, date difference and D-day countdown.',
    'page.aboutTitle': 'About this site',
    'page.privacyTitle': 'Privacy and data handling',
    'page.contactTitle': 'Contact and feedback',
    'common.languageLabel': 'Language',
    'common.languageKorean': '한국어',
    'common.languageEnglish': 'English',
    'common.copy': 'Copy',
    'common.copySuccess': 'Copied to clipboard.',
    'common.copyFail': 'Failed to copy. Check your browser permissions.',
    'common.clear': 'Clear',
    'common.sample': 'Sample',
    'common.back': '← Back to tools',
    'common.shareState': 'Copy shareable link',
    'common.snippetTitle': 'CLI · Code Snippet',
    'common.snippetSubtitle': 'Node.js · shell',
    'common.footerHome': `© ${YEAR} stateless tools · Static site for Cloudflare Pages deployment`,
    'common.footerBase64': `© ${YEAR} stateless tools · All conversion stays inside your browser. No files/cookies sent.`,
    'common.footerUuid': `© ${YEAR} stateless tools · Static site for Cloudflare Pages deployment`,
    'common.footerJson': `© ${YEAR} stateless tools · JSON stays entirely in browser memory.`,
    'common.footerJwt': `© ${YEAR} stateless tools · JWT decoding and JWKS checks run in your browser.`,
    'common.footerSeo': `© ${YEAR} stateless tools · SEO checks run from a URL or pasted HTML in your browser.`,
    'common.yes': 'Yes',
    'common.no': 'No',
    'upload.eyebrow': 'Upload',
    'upload.titleDefault': 'File upload',
    'upload.choose': 'Choose file',
    'upload.replace': 'Replace file',
    'upload.clear': 'Clear selection',
    'upload.none': 'No file selected yet.',
    'upload.singleSummary': '{name} · {size}',
    'upload.multiSummary': '{count} files · {size} total',
    'upload.extraFiles': '{count} more file(s)',
    'upload.hintAny': 'Drag and drop a local file here or browse from this device.',
    'upload.hintImage': 'Drag PNG, JPG, WEBP, or SVG files here or browse from this device.',
    'upload.hintPdfSingle': 'Drop a PDF here or browse from this device.',
    'upload.hintPdfMany': 'Drop multiple PDFs here or browse from this device.',
    'home.eyebrow': 'stateless dev tools',
    'home.title': 'Developer Tools',
    'home.description': 'Use browser-based developer tools for UUID, Base64, JSON, JWT, PDF, and more.',
    'home.card.aihub.title': 'AI Command Cheatsheets',
    'home.card.aihub.desc': 'Current Claude Code and Codex command flows with official docs.',
    'home.card.claude.title': 'Claude Code Commands',
    'home.card.claude.desc': 'Commands, skills, MCP, hooks, run/verify, and background agents.',
    'home.card.codex.title': 'Codex Commands',
    'home.card.codex.desc': 'Codex CLI, slash commands, AGENTS.md, Skills, MCP, and Automations.',
    'home.card.uuid.title': 'UUID Converter',
    'home.card.uuid.desc': 'Random UUIDs plus round-trip hex(binary16) conversion.',
    'home.card.base64.title': 'Base64 Encoder/Decoder',
    'home.card.base64.desc': 'Encode or decode Base64 text, files, and URL-safe values.',
    'home.card.json.title': 'JSON Formatter',
    'home.card.json.desc': 'Format, validate, minify, and inspect JSON in tree view.',
    'home.card.jwt.title': 'JWT Decoder',
    'home.card.jwt.desc': 'Decode JWTs, inspect claims, and check expiry quickly.',
    'home.card.url.title': 'URL Encoder/Decoder',
    'home.card.url.desc': 'Encode, decode, and inspect query string parameters.',
    'home.card.hash.title': 'SHA Hash Calculator',
    'home.card.hash.desc': 'Compute SHA hashes and copy HEX or Base64 output.',
    'home.card.cron.title': 'Cron Parser',
    'home.card.cron.desc': 'Parse cron expressions and preview next run times.',
    'timestamp.title': 'Unix Timestamp Converter',
    'timestamp.description': 'Convert between Unix timestamps and human-readable dates.',
    'timestamp.current.title': 'Current Time',
    'timestamp.current.unix': 'Unix Timestamp (Seconds)',
    'timestamp.current.iso': 'ISO 8601',
    'timestamp.current.stop': 'Stop Live Update',
    'timestamp.current.start': 'Start Live Update',
    'timestamp.convert.title': 'Converter',
    'timestamp.input.label': 'Unix Timestamp',
    'timestamp.input.current': 'Current',
    'timestamp.unit.seconds': 'Seconds',
    'timestamp.unit.millis': 'Milliseconds',
    'timestamp.output.local': 'Local Date',
    'timestamp.output.iso': 'ISO / UTC',
    'timestamp.output.relative': 'Relative',
    'timestamp.reverse.title': 'Date → Timestamp',
    'timestamp.reverse.button': 'Convert',
    'home.card.timestamp.title': 'Unix Timestamp Converter',
    'home.card.timestamp.desc': 'Convert epoch seconds or milliseconds to dates and back.',
    'home.card.password.title': 'Password Generator',
    'home.card.password.desc': 'Generate secure passwords with flexible length and character options.',
    'home.card.regex.title': 'Regex Tester',
    'home.card.regex.desc': 'Test patterns and flags, then inspect highlighted matches.',
    'home.card.qr.title': 'QR Code Generator/Scanner',
    'home.card.qr.desc': 'Generate text/Wi-Fi QR and scan QR from image uploads.',
    'home.card.diff.title': 'Text Diff',
    'home.card.diff.desc': 'Compare original/edited lines with unified diff output.',
    'home.card.color.title': 'Color Converter/Contrast Checker',
    'home.card.color.desc': 'Convert HEX, RGB, HSL and check contrast ratios.',
    'home.card.markdown.title': 'Markdown Preview',
    'home.card.markdown.desc': 'Render Markdown and extract HTML.',
    'home.card.convert.title': 'Unit Converter',
    'home.card.convert.desc': 'Quick conversion for length/weight/temperature.',
    'home.card.filehash.title': 'File Hash Calculator',
    'home.card.filehash.desc': 'Compute SHA hashes in your browser.',
    'home.card.imagebase64.title': 'Image Base64 Converter',
    'home.card.imagebase64.desc': 'Convert image files to/from data URLs.',
    'home.card.case.title': 'Case Converter',
    'home.card.case.desc': 'Convert camelCase, snake_case, kebab-case, and PascalCase.',
    'home.card.jsonyaml.title': 'JSON YAML Converter',
    'home.card.jsonyaml.desc': 'Convert JSON and YAML, plus clean up CSV data.',
    'home.card.query.title': 'URL Query Builder',
    'home.card.query.desc': 'Build query strings with key/value rows.',
    'home.card.ipua.title': 'IP/User-Agent Info',
    'home.card.ipua.desc': 'Quick check for IP, UA, locale and timezone.',
    'home.card.textstats.title': 'Character Counter',
    'home.card.textstats.desc': 'Characters with and without spaces, words, sentences, paragraphs.',
    'home.card.ipcidr.title': 'CIDR Calculator',
    'home.card.ipcidr.desc': 'Calculate subnet, broadcast, and host ranges from IPv4 CIDR blocks.',
    'home.card.pdf.title': 'PDF Merge/Split',
    'home.card.pdf.desc': 'Merge, split, extract pages, watermark, and inspect PDFs.',
    'home.card.imageoptimize.title': 'Image Optimizer',
    'home.card.imageoptimize.desc': 'Compress, resize, and convert WEBP/JPEG/PNG files.',
    'home.card.ocr.title': 'OCR Text Extractor',
    'home.card.ocr.desc': 'Extract Korean and English text from images.',
    'home.card.seocheck.title': 'SEO Meta Tag Checker',
    'home.card.seocheck.desc': 'Audit title, description, OG, and canonical tags.',
    'home.card.utm.title': 'UTM Generator',
    'home.card.utm.desc': 'Build campaign URLs and test short-code style parameters.',
    'home.card.textcleaner.title': 'Text Cleaner',
    'home.card.textcleaner.desc': 'Remove duplicates, trim whitespace, sort lines, and slugify.',
    'home.card.apitester.title': 'API Request Tester',
    'home.card.apitester.desc': 'Send HTTP requests, inspect responses, and export them as cURL.',
    'base64.title': 'Base64 Encoder/Decoder',
    'base64.description': 'Encode and decode Base64 strings, URL-safe Base64, and files directly in the browser.',
    'base64.encode.title': 'Text → Base64',
    'base64.encode.input': 'Text input',
    'base64.encode.placeholder': 'e.g. Hello, world',
    'base64.encode.urlSafe': 'URL-safe output',
    'base64.encode.button': 'Encode',
    'base64.encode.result': 'Base64 result',
    'base64.decode.title': 'Base64 → Text',
    'base64.decode.input': 'Base64 input',
    'base64.decode.placeholder': 'e.g. SGVsbG8sIHdvcmxkIQ==',
    'base64.decode.strip': 'Strip whitespace',
    'base64.decode.urlSafe': 'Restore URL-safe',
    'base64.decode.button': 'Decode',
    'base64.decode.result': 'Text result',
    'base64.file.title': 'File ↔ Base64',
    'base64.file.helper': 'Drop any file to convert it into Base64 immediately.',
    'base64.file.downloadPlaceholder': 'decoded.bin',
    'base64.file.downloadBtn': 'Export Base64 to file',
    'uuid.title': 'UUID Converter',
    'uuid.description': 'crypto.randomUUID() generation plus hex(binary16) round-trips.',
    'uuid.random.label': 'Random UUID',
    'uuid.random.button': 'Generate',
    'uuid.random.mode': 'Generation mode',
    'uuid.random.v4': 'UUID v4',
    'uuid.random.v7': 'UUID v7 (time ordered)',
    'uuid.random.ulid': 'ULID (readable)',
    'uuid.toHex.title': 'UUID → Hex(binary16)',
    'uuid.toHex.input': 'UUID input',
    'uuid.toHex.placeholder': 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx',
    'uuid.toHex.button': 'Convert',
    'uuid.toHex.output': 'Hex output',
    'uuid.toUuid.title': 'Hex(binary16) → UUID',
    'uuid.toUuid.input': 'Hex input (32 chars)',
    'uuid.toUuid.placeholder': '32 hex characters',
    'uuid.toUuid.button': 'Restore',
    'uuid.toUuid.output': 'UUID output',
    'uuid.bulk.title': 'Bulk generation',
    'uuid.bulk.count': 'Count',
    'uuid.bulk.button': 'Generate',
    'json.title': 'JSON Formatter/Validator',
    'json.description': 'Format, validate, minify, and inspect JSON with tree view support.',
    'json.input': 'JSON input',
    'json.placeholder': '{"hello":"world"}',
    'json.format': 'Format',
    'json.minify': 'Minify',
    'json.output': 'Output',
    'json.searchLabel': 'Search keys/values',
    'json.searchPlaceholder': 'user.id',
    'json.highlightTitle': 'Highlighted JSON',
    'json.treeTitle': 'Tree view',
    'json.treeCollapse': 'Collapse all',
    'json.treeNode.array': 'Array',
    'json.treeNode.object': 'Object',
    'json.meta.empty': 'No input',
    'messages.base64.encodeSuccess': 'Encoded text to Base64.',
    'messages.base64.encodeError': 'Failed to encode. Please check the input.',
    'messages.base64.decodeSuccess': 'Decoded Base64 back to text.',
    'messages.base64.decodeError': 'Failed to decode Base64. Please verify the value.',
    'messages.base64.fileEncoded': 'Converted {name} to Base64.',
    'messages.base64.fileDecoded': 'Exported Base64 as a file.',
    'messages.base64.fileError': 'File conversion failed.',
    'messages.uuid.randomGenerated': 'Generated a fresh UUID.',
    'messages.uuid.toHexSuccess': 'Converted UUID to hex(binary16).',
    'messages.uuid.toHexError': 'Invalid UUID format.',
    'messages.uuid.toUuidSuccess': 'Recovered UUID from hex(binary16).',
    'messages.uuid.toUuidError': 'Hex(binary16) must be a 32-character hex string.',
    'messages.uuid.bulkGenerated': 'Generated {count} identifiers.',
    'messages.json.empty': 'JSON input is empty.',
    'messages.json.pretty': 'Pretty-printed the JSON.',
    'messages.json.compact': 'Minified the JSON.',
    'messages.json.error': 'JSON parse failed: {message}',
    'messages.json.cleared': 'Reset the input.',
    'messages.json.sample': 'Loaded a sample JSON.',
    'json.meta.array': 'Array · {count} items',
    'json.meta.object': 'Object · {count} keys ({examples})',
    'json.meta.primitive': 'Primitive · {type}',
    'jwt.title': 'JWT Decoder/Inspector',
    'jwt.description': 'Decode JWTs, inspect claims, review expiry, and verify signatures with JWKS.',
    'jwt.input': 'JWT token',
    'jwt.placeholder': 'eyJhbGciOi...',
    'jwt.action.decode': 'Decode',
    'jwt.output.header': 'Header',
    'jwt.output.payload': 'Payload',
    'jwt.success': 'Decoded JWT successfully.',
    'jwt.error.empty': 'Please paste a JWT.',
    'jwt.error.format': 'Invalid JWT format.',
    'jwt.error.decode': 'Failed to decode JWT.',
    'jwt.meta.expires': 'expires: {date}',
    'jwt.meta.expired': 'expired: {date}',
    'jwt.meta.nbfActive': 'nbf passed: {date}',
    'jwt.meta.nbfPending': 'nbf pending: {date}',
    'cron.title': 'Cron Parser',
    'cron.description': 'Parse cron expressions and preview the next run times.',
    'cron.input': 'Cron Expression',
    'cron.nextRuns': 'Next 5 Runs',
    'cron.empty': 'Enter a cron expression, e.g. */5 * * * *',
    'cron.error.invalid': 'Invalid cron expression.',
    'cron.error.details': 'Details: {message}',
    'url.title': 'URL Encoder/Decoder',
    'url.description': 'Encode or decode URLs and inspect query string parameters.',
    'url.tab.convert': 'Encode / Decode',
    'url.tab.build': 'Query builder',
    'url.input': 'Original string/URL',
    'url.placeholder': 'https://example.com?q=Hello',
    'url.encode': 'Encode',
    'url.decode': 'Decode',
    'url.output': 'Result',
    'url.outputHelper': 'Copy or inspect the parameter table.',
    'url.paramsTitle': 'Query parameters',
    'url.paramsEmpty': 'No parameters to display.',
    'url.success.encode': 'Encoded the value.',
    'url.success.decode': 'Decoded the value.',
    'url.error.empty': 'Enter a value first.',
    'url.error.encode': 'Encoding failed.',
    'url.error.decode': 'Decoding failed.',
    'hash.title': 'SHA Hash Calculator',
    'hash.description': 'Compute SHA-1, SHA-256, SHA-384, and SHA-512 hashes via WebCrypto.',
    'hash.input': 'Input text',
    'hash.placeholder': 'stateless tools',
    'hash.algorithm': 'Algorithm',
    'hash.uppercase': 'Uppercase HEX output',
    'hash.compute': 'Compute hash',
    'hash.hex': 'HEX',
    'hash.base64': 'Base64',
    'hash.error.empty': 'Input text is required.',
    'hash.error.unsupported': 'Unable to compute hash in this browser.',
    'hash.success': 'Generated {algorithm} hash.',
    'password.title': 'Password Generator',
    'password.description': 'Generate secure passwords instantly and inspect estimated entropy strength.',
    'password.length': 'Length',
    'password.count': 'Count',
    'password.option.lower': 'Lowercase (a-z)',
    'password.option.upper': 'Uppercase (A-Z)',
    'password.option.numbers': 'Numbers (0-9)',
    'password.option.symbols': 'Symbols',
    'password.option.excludeAmbiguous': 'Exclude ambiguous chars (0/O, l/1)',
    'password.generate': 'Generate',
    'password.output': 'Generated passwords',
    'password.strength.label': 'Strength: {level} ({bits} bits)',
    'password.strength.weak': 'Weak',
    'password.strength.fair': 'Fair',
    'password.strength.good': 'Good',
    'password.strength.strong': 'Strong',
    'password.strength.excellent': 'Excellent',
    'password.error.charset': 'Pick at least one character set.',
    'password.success.generated': 'Generated {count} passwords.',
    'regex.title': 'Regex Tester',
    'regex.description': 'Test regex patterns, flags, and replacements against your input text.',
    'regex.pattern': 'Pattern',
    'regex.pattern.placeholder': '\\b\\w+@\\w+\\.\\w+\\b',
    'regex.flags': 'Flags',
    'regex.input': 'Input text',
    'regex.test': 'Test',
    'regex.output': 'Highlighted output',
    'regex.matches': 'Match list',
    'regex.error.pattern': 'Enter a regex pattern.',
    'regex.error.invalid': 'Regex error: {message}',
    'regex.result.none': 'No matches found.',
    'regex.result.count': '{count} matches found.',
    'case.title': 'Case Converter',
    'case.description': 'Convert camelCase, snake_case, kebab-case and PascalCase.',
    'case.input': 'Input text',
    'case.run': 'Convert',
    'case.error.empty': 'Enter text first.',
    'case.success': 'Converted successfully.',
    'jsonyaml.title': 'JSON ↔ YAML Converter',
    'jsonyaml.description': 'Convert JSON and YAML while moving structured data between formats.',
    'jsonyaml.input': 'Input',
    'jsonyaml.output': 'Output',
    'jsonyaml.toYaml': 'JSON → YAML',
    'jsonyaml.toJson': 'YAML → JSON',
    'jsonyaml.success.toYaml': 'Converted JSON to YAML.',
    'jsonyaml.success.toJson': 'Converted YAML to JSON.',
    'jsonyaml.error': 'Conversion failed: {message}',
    'query.base': 'Base URL',
    'query.add': 'Add parameter',
    'query.build': 'Build URL',
    'query.output': 'Output URL',
    'query.remove': 'Remove',
    'query.success': 'URL built successfully.',
    'query.error.base': 'Invalid base URL.',
    'ipua.title': 'IP/User-Agent Info',
    'ipua.description': 'Check IP, user-agent, language, and timezone quickly.',
    'ipua.externalNote': 'A browser cannot resolve its own public IP, so this page calls api.ipify.org, which exposes your IP to that service. User-Agent, language, timezone and platform are read locally.',
    'ipua.refresh': 'Refresh',
    'ipua.lang': 'Language',
    'ipua.tz': 'Timezone',
    'ipua.platform': 'Platform',
    'ipua.success': 'Info refreshed.',
    'ipua.error.ip': 'Failed to fetch IP (network/CORS).',
    'textStats.title': 'Character Counter',
    'textStats.description': 'Counts characters with and without spaces, words, sentences and paragraphs as you type.',
    'textStats.input': 'Input text',
    'textStats.run': 'Analyze',
    'textStats.sample': 'Sample',
    'textStats.resultTitle': 'Statistics',
    'textStats.charsWithSpaces': 'Characters (with spaces)',
    'textStats.charsNoSpaces': 'Characters (without spaces)',
    'textStats.words': 'Words',
    'textStats.lines': 'Lines',
    'textStats.paragraphs': 'Paragraphs',
    'textStats.sentences': 'Sentences (heuristic)',
    'textStats.readingTime': 'Estimated reading time',
    'textStats.bytes': 'Bytes',
    'textStats.manuscript': 'Manuscript pages (200)',
    'textStats.uniqueWords': 'Unique words',
    'textStats.success': 'Text analysis complete.',
    'ipCidr.title': 'CIDR Calculator',
    'ipCidr.description': 'Compute subnet, broadcast, and host range details from IPv4 CIDR blocks.',
    'ipCidr.ip': 'IPv4 Address',
    'ipCidr.prefix': 'CIDR Prefix',
    'ipCidr.calculate': 'Calculate',
    'ipCidr.copy': 'Copy CIDR',
    'ipCidr.output': 'Result',
    'ipCidr.network': 'Network',
    'ipCidr.broadcast': 'Broadcast',
    'ipCidr.netmask': 'Subnet mask',
    'ipCidr.wildcard': 'Wildcard mask',
    'ipCidr.usable': 'Usable hosts',
    'ipCidr.class': 'Address type',
    'ipCidr.firstHost': 'First host',
    'ipCidr.lastHost': 'Last host',
    'ipCidr.total': 'Total addresses',
    'ipCidr.isNetwork': 'Input IP is network address',
    'ipCidr.isBroadcast': 'Input IP is broadcast address',
    'ipCidr.error.ip': 'Enter a valid IPv4 address.',
    'ipCidr.error.prefix': 'Prefix must be between 0 and 32.',
    'ipCidr.success': 'CIDR calculation complete.',
    'qr.title': 'QR Code Generator/Scanner',
    'qr.description': 'Generate QR codes and scan uploaded QR images in the browser.',
    'qr.type': 'Type',
    'qr.type.text': 'Text/URL',
    'qr.type.wifi': 'Wi-Fi',
    'qr.input': 'Text/URL',
    'qr.ssid': 'SSID',
    'qr.password': 'Password',
    'qr.security': 'Security',
    'qr.security.none': 'None',
    'qr.size': 'Size (px)',
    'qr.generate': 'Generate',
    'qr.download': 'Download PNG',
    'qr.scanFile': 'Scan from image',
    'qr.scanOutput': 'Scan result',
    'qr.error.empty': 'Enter text first.',
    'qr.error.lib': 'Failed to load QR library.',
    'qr.error.generate': 'Failed to generate QR code.',
    'qr.success.generated': 'QR code generated.',
    'qr.error.unsupported': 'This browser does not support QR scanning.',
    'qr.error.notFound': 'No QR code found.',
    'qr.success.scanned': 'QR scan completed.',
    'qr.error.scan': 'An error occurred while scanning QR.',
    'diff.title': 'Text Diff Checker',
    'diff.description': 'Compare two text blocks line by line.',
    'diff.left': 'Original',
    'diff.right': 'Modified',
    'diff.compare': 'Compare',
    'diff.result': 'Result',
    'diff.unified': 'Unified Diff',
    'diff.result.changed': '{count} changed lines',
    'diff.result.same': 'No differences found.',
    'color.title': 'Color Converter/Contrast Checker',
    'color.description': 'Convert HEX, RGB, and HSL colors while checking contrast ratios.',
    'color.pick': 'Pick color',
    'color.contrast': 'Contrast ratio',
    'color.contrast.value': 'vs White: {white} / vs Black: {black}',
    'color.success': 'Color values updated.',
    'color.error.parse': 'Failed to parse color.',
    'markdown.title': 'Markdown Preview',
    'markdown.description': 'Render Markdown and inspect the resulting HTML output.',
    'markdown.input': 'Markdown',
    'markdown.render': 'Render',
    'markdown.preview': 'Preview',
    'markdown.error.lib': 'Failed to load Markdown library.',
    'markdown.success.render': 'Rendered successfully.',
    'markdown.error.render': 'Render failed.',
    'convert.title': 'Unit Converter',
    'convert.description': 'Convert common length, weight, and temperature units quickly.',
    'convert.category': 'Category',
    'convert.category.length': 'Length',
    'convert.category.weight': 'Weight',
    'convert.category.temp': 'Temperature',
    'convert.input': 'Input value',
    'convert.from': 'From',
    'convert.to': 'To',
    'convert.swap': 'Swap',
    'convert.run': 'Convert',
    'convert.output': 'Output',
    'convert.error.nan': 'Enter a numeric value.',
    'convert.success': 'Conversion completed.',
    'filehash.title': 'File Hash Calculator',
    'filehash.description': 'Calculate SHA hashes for uploaded files locally in the browser.',
    'filehash.file': 'File',
    'filehash.algorithm': 'Algorithm',
    'filehash.run': 'Compute',
    'filehash.error.file': 'Select a file first.',
    'filehash.success': '{algorithm} computed ({name})',
    'filehash.error.run': 'Failed to compute hash.',
    'imagebase64.title': 'Image Base64 Converter',
    'imagebase64.description': 'Convert image files to and from Base64 data URLs.',
    'imagebase64.file': 'Image file',
    'imagebase64.encode': 'Convert to Base64',
    'imagebase64.decode': 'Save as file',
    'imagebase64.output': 'Data URL',
    'imagebase64.error.file': 'Select an image first.',
    'imagebase64.success.encode': 'Converted to Base64.',
    'imagebase64.error.encode': 'Conversion failed.',
    'imagebase64.error.format': 'Invalid data URL format.',
    'imagebase64.success.decode': 'File saved.',
    'pdf.title': 'PDF Merge/Split Toolkit',
    'pdf.description': 'Merge, split, extract pages, watermark, and inspect PDFs in the browser.',
    'pdf.mergeSection': 'Merge',
    'pdf.mergeDescription': 'Upload multiple PDFs and merge them in order.',
    'pdf.mergeFiles': 'PDFs to merge',
    'pdf.singleSection': 'Single PDF operations',
    'pdf.singleDescription': 'Run split, watermark, metadata, and text extraction on one PDF.',
    'pdf.file': 'PDF File',
    'pdf.mergeBtn': 'Merge',
    'pdf.splitBtn': 'Extract range',
    'pdf.watermarkBtn': 'Watermark',
    'pdf.metadataBtn': 'Metadata',
    'pdf.extractTextBtn': 'Extract text',
    'pdf.fromPage': 'From page',
    'pdf.toPage': 'To page',
    'pdf.watermarkText': 'Watermark text',
    'pdf.dropHint': 'Drop files here.',
    'pdf.mergeDropHint': 'Or click to choose files.',
    'pdf.singleDropHint': 'Or click to choose files.',
    'pdf.fileNone': 'No file selected.',
    'pdf.fileSummaryOne': '{name}',
    'pdf.fileSummaryMany': '{count} files selected',
    'pdf.moreFiles': 'and {count} more',
    'pdf.fileSummaryHeader': '{count} files',
    'pdf.fileSizeLabel': 'Total {size}',
    'pdf.fileMeta': '{name} · {size}',
    'pdf.output': 'Output',
    'pdf.downloadBtn': 'Download',
    'pdf.page': 'Page',
    'pdf.emptyText': '(No text)',
    'pdf.meta.filename': 'Filename',
    'pdf.meta.size': 'Size',
    'pdf.meta.pages': 'Page count',
    'pdf.meta.encrypted': 'Encrypted',
    'pdf.meta.title': 'Title',
    'pdf.meta.author': 'Author',
    'pdf.meta.subject': 'Subject',
    'pdf.meta.keywords': 'Keywords',
    'pdf.meta.creator': 'Creator',
    'pdf.meta.producer': 'Producer',
    'pdf.meta.created': 'Created',
    'pdf.meta.modified': 'Modified',
    'pdf.meta.valueUnknown': 'Unknown',
    'pdf.meta.valueNone': 'N/A',
    'messages.pdf.needTwoFiles': 'Select at least 2 PDF files to merge.',
    'messages.pdf.requireOneFile': 'Select one PDF file first.',
    'messages.pdf.invalidRange': 'The start page cannot be greater than the end page.',
    'messages.pdf.emptyWatermark': 'Enter watermark text.',
    'messages.pdf.mergeDone': 'PDF merge complete.',
    'messages.pdf.splitDone': 'Page range extraction complete.',
    'messages.pdf.watermarkDone': 'Watermark applied.',
    'messages.pdf.metadataDone': 'Metadata read complete.',
    'messages.pdf.extractDone': 'Extracted text from pages {from}~{to}.',
    'messages.pdf.singleFileOnly': 'Single-file PDF operations require only 1 file.',
    'messages.pdf.invalidFileType': 'Non-PDF files were ignored. Only PDF files are allowed.',
    'messages.pdf.mergeSuccess': 'Merged {count} files with a total of {pages} pages.',
    'messages.pdf.splitSummary': '{from}~{to} / total {total} pages.',
    'messages.pdf.watermarkSummary': 'Applied to {pages} pages.',
    'messages.pdf.mergeError': 'Failed to merge PDF files.',
    'messages.pdf.splitError': 'Failed to extract PDF range.',
    'messages.pdf.watermarkError': 'Failed to apply watermark.',
    'messages.pdf.metadataError': 'Failed to read PDF metadata.',
    'messages.pdf.extractError': 'Failed to extract PDF text.',
    'api.title': 'API Request Tester',
    'api.description': 'Send HTTP requests, inspect responses, and export them as cURL commands.',
    'api.headers': 'Headers (Key: Value)',
    'api.body': 'Body',
    'api.send': 'Send Request',
    'api.response': 'Response',
    'messages.api.urlRequired': 'Enter a request URL.',
    'messages.api.done': 'API request complete.',
    'messages.api.failed': 'Request failed: {message}',
    'imageOptimize.title': 'Image Optimizer',
    'imageOptimize.description': 'Compress, resize, and convert JPEG, PNG, and WEBP files in the browser.',
    'imageOptimize.file': 'Image file',
    'imageOptimize.maxWidth': 'Max width (px)',
    'imageOptimize.quality': 'Quality (0.2~1)',
    'imageOptimize.format': 'Output format',
    'imageOptimize.run': 'Optimize',
    'imageOptimize.download': 'Download',
    'imageOptimize.before': 'Original',
    'imageOptimize.after': 'Optimized output',
    'imageOptimize.preview': 'Preview',
    'messages.imageOptimize.selectFile': 'Choose an image file first.',
    'messages.imageOptimize.done': 'Image optimization complete.',
    'messages.imageOptimize.failed': 'Failed to optimize the image.',
    'messages.imageOptimize.reduced': 'reduced',
    'ocr.title': 'OCR Text Extractor',
    'ocr.description': 'Extract Korean and English text from images.',
    'ocr.file': 'Image file',
    'ocr.language': 'Language',
    'ocr.langBoth': 'Korean + English',
    'ocr.langEnglish': 'English',
    'ocr.langKorean': 'Korean',
    'ocr.run': 'Run OCR',
    'ocr.output': 'Extracted text',
    'messages.ocr.selectFile': 'Choose an image for OCR.',
    'messages.ocr.loading': 'Loading the OCR engine.',
    'messages.ocr.done': 'OCR complete.',
    'messages.ocr.failed': 'Failed to process OCR.',
    'seo.title': 'SEO Meta Tag Checker',
    'seo.description': 'Inspect title, description, OG, and canonical tags from a URL or pasted HTML.',
    'seo.tab.serp': 'Title & description',
    'seo.tab.html': 'Full HTML check',
    'seo.titleLabel': 'Title tag',
    'seo.descLabel': 'Meta description',
    'seo.urlLabel': 'Display URL (optional)',
    'seo.previewLabel': 'Search result preview',
    'seo.liveNote': 'Recalculated as you type — no button to press.',
    'seo.html': 'HTML source (optional)',
    'seo.run': 'Check',
    'seo.output': 'Result',
    'seo.report.title': 'Title',
    'seo.report.description': 'Description',
    'seo.report.h1': 'H1',
    'seo.report.canonical': 'Canonical',
    'seo.report.ogTitle': 'OG Title',
    'seo.report.ogDescription': 'OG Description',
    'seo.report.robots': 'Robots',
    'seo.report.jsonLdBlocks': 'JSON-LD blocks',
    'messages.seo.fetchFailed': 'Failed to fetch the URL directly, possibly due to CORS. Paste the HTML source instead.',
    'messages.seo.needInput': 'Enter a URL or paste HTML source.',
    'messages.seo.done': 'SEO check complete.',
    'messages.seo.failed': 'Failed to run the SEO check.',
    'utm.title': 'UTM Generator',
    'utm.description': 'Build campaign URLs with UTM parameters for ads, newsletters, and social posts.',
    'utm.build': 'Build URL',
    'utm.shortOutput': 'Short URL (simulation)',
    'messages.utm.done': 'UTM URL generated successfully.',
    'messages.utm.invalidBase': 'The base URL is invalid.',
    'messages.utm.failed': 'Failed to generate the UTM URL.',
    'textCleaner.title': 'Text Cleaner',
    'textCleaner.description': 'Trim whitespace, remove duplicates, sort lines, and slugify in one pass.',
    'textCleaner.input': 'Input text',
    'textCleaner.trim': 'Trim whitespace',
    'textCleaner.removeEmpty': 'Remove empty lines',
    'textCleaner.dedupe': 'Remove duplicate lines',
    'textCleaner.sort': 'Sort lines',
    'textCleaner.slug': 'Convert to slug',
    'textCleaner.run': 'Clean text',
    'textCleaner.output': 'Output',
    'messages.textCleaner.done': 'Text cleaning complete.',
    'textStats.placeholder': 'Paste text here.',
    'textStats.sampleText': 'Hello there. Quick brown fox jumps over the lazy dog.\n\nThis is sample text for counting characters and words.\nMultiple lines and sentences are included.',
    'textStats.readingTimeValue': '{minutes} min {seconds} sec',
    'jsonyaml.success.csvToJson': 'Converted CSV to JSON.',
    'jsonyaml.success.jsonToCsv': 'Converted JSON to CSV.',
    'jwt.verify.enterJwks': 'Enter a JWKS URL.',
    'jwt.verify.noKey': 'No matching key was found in the JWKS.',
    'jwt.verify.success': 'JWT signature verified successfully.',
    'jwt.verify.failed': 'JWT signature verification failed: {message}',
  },
};
