  ...
  '/query-builder': {
    ko: {
      examples: [
        {
          title: '기본 파라미터 조합',
          input: 'base   https://api.example.com/search\nq      개발자 도구\npage   2',
          output: 'https://api.example.com/search?q=%EA%B0%9C%EB%B0%9C%EC%9E%90%20%EB%8F%84%EA%B5%AC&page=2',
        },
        {
          title: '특수문자가 든 값',
          input: 'redirect  https://example.com/back?id=1&t=2',
          output: 'redirect=https%3A%2F%2Fexample.com%2Fback%3Fid%3D1%26t%3D2',
          note: '값 안의 &와 =까지 인코딩해야 파라미터가 끊기지 않습니다.',
        },
      ],
      troubles: [
        {
          title: '공백이 +인지 %20인지',
          body: '쿼리 문자열에서는 둘 다 공백으로 해석되는 것이 보통이지만, 경로(path)에서는 +가 문자 그대로 플러스입니다. 서버 구현에 따라 다르게 받으므로 %20이 안전합니다.',
        },
        {
          title: '배열 파라미터 표기',
          body: 'tags=a&tags=b, tags[]=a&tags[]=b, tags=a,b 세 가지가 모두 쓰입니다. 어느 것을 받는지는 서버 프레임워크에 달렸으니 API 문서를 먼저 확인하세요.',
        },
      ],
    },
    en: {
      examples: [
        {
          ...
          input: 'base   https://api.example.com/search\nq      developer tools\npage   2',
          output: 'https://api.example.com/search?q=developer%20tools&page=2',
        },
        {
          title: 'A value containing special characters',
          input: 'redirect  https://example.com/back?id=1&t=2',
          output: 'redirect=https%3A%2F%2Fexample.com%2Fback%3Fid%3D1%26t%3D2',
          note: 'The & and = inside the value must be encoded or the parameter list breaks apart.',
        },
      ],
      troubles: [
        {
          title: 'Is a space + or %20?',
          body: 'Both usually decode to a space in a query string, but in a path a + is a literal plus. Servers differ, so %20 is the safer choice.',
        },
        {
          title: 'Array parameter styles',
          body: 'tags=a&tags=b, tags[]=a&tags[]=b, and tags=a,b are all in use. Which one works depends on the server framework — check the API docs first.',
        },
      ],
    },
  },
      ...
      examples: [
        {
          title: '연속 생성 시 정렬',
          input: '3회 연속 생성',
          output: '01923f8a-1c4d-7000-8a3b-9f2e1d4c5b6a\n01923f8a-1c51-7000-8a3b-2c7d9e4f1a3b\n01923f8a-1c55-7000-8a3b-6b1a8c2d7e9f',
          note: '앞 12자리가 밀리초 타임스탬프라 문자열 정렬만으로도 생성 순서가 유지됩니다.',
        },
      ],
      troubles: [
        {
          title: 'v4를 쓰던 인덱스가 느렸던 이유',
          body: 'v4는 완전 난수라 B-tree 인덱스에 무작위로 삽입되어 페이지 분할이 잦습니다. v7은 시간순이라 끝에 붙어 삽입 성능과 캐시 적중률이 낫습니다.',
        },
        {
          title: '생성 시각이 노출됩니다',
          body: 'v7에는 밀리초 단위 생성 시각이 그대로 들어 있습니다. 가입 시점이나 주문 시각을 숨겨야 하는 공개 식별자에는 v4가 맞습니다.',
        },
      ],
    },
    en: {
      examples: [
        {
          title: 'Sort order across sequential IDs',
          input: 'Generate three in a row',
          output: '01923f8a-1c4d-7000-8a3b-9f2e1d4c5b6a\n01923f8a-1c51-7000-8a3b-2c7d9e4f1a3b\n01923f8a-1c55-7000-8a3b-6b1a8c2d7e9f',
          note: 'The first 12 hex digits are a millisecond timestamp, so plain string sorting preserves creation order.',
        },
      ],
      troubles: [
        {
          title: 'Why v4 made your index slow',
          body: 'v4 is fully random, so rows land anywhere in a B-tree index and split pages often. v7 appends in time order, which improves insert throughput and cache hit rate.',
        },
        {
          title: 'The creation time is exposed',
          body: 'A v7 value embeds its millisecond creation time. If a public identifier must not reveal signup or order timing, use v4 instead.',
        },
      ],
    },
  },
